<link rel="shortcut icon" type="image/png" href="favicon.png"/>StudentStudentStudentStudent
<?php
session_start();
include 'connect.php';
	...
	if($result->num_rows > 0)
		{
			while($row = $result->fetch_assoc())
				{
					$sql3 = "SELECT Dept,SickLeave,EarnLeave,CasualLeave FROM students WHERE Dept = '".$row["Dept"]."'";
							...
							while($row2 = $result2->fetch_assoc())
								{
									if($row2["EarnLeave"] == $row["SetEarnLeave"])
										{
											$update = "UPDATE students SET EarnLeave = '".$setearnleave."' WHERE Dept = '".$row2["Dept"]."'";
											$conn->query($update);
										}
									if($row2["SickLeave"] == $row["SetSickLeave"])
										{
											$update = "UPDATE students SET SickLeave = '".$setsickleave."'WHERE Dept = '".$row2["Dept"]."'";
											$conn->query($update);
										}
									if($row2["CasualLeave"] == $row["SetCasualLeave"])
										{
											$update = "UPDATE students SET CasualLeave = '".$setcasualleave."' WHERE Dept = '".$row2["Dept"]."'";
											$conn->query($update);
										}
								}
						}
