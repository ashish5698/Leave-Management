...
include 'navi.php';
$sql = "SELECT id, UserName, stuPass, stuEmail FROM Students";
...
if(isset($uname) && isset($to))
	{$rand = mt_rand();
		$sql2 = "UPDATE Students SET Random='".$rand."' WHERE id=".$id;
		$conn->query($sql2);
...
if(stuty($key))
	{
	header('location:passrecovery.php?err='.urlencode('stuty Field !'));
	}
?>
