						...
						if($row["UserName"] == $user)
							{
								if($leavetype === "Sick Leave")
								{
									if(($leavedays <= $row["SickLeave"]) || $leavedays < 0)
										...
										$stuname = $row["StuName"];
										$to = $row["StuEmail"];
										$sql2 = "INSERT INTO stu_leaves(StuName,LeaveReason,LeaveDays,StartDate,EndDate,Dept) VALUES('".$stuname."','".$leavereason."','".$leavedays."','".$leavedate."','".$end."','".$row['Dept']."')";
									...
									{
									header('location:leaverequest.php?err='.urlencode("You cannot ask for leaves more than that of your account !"));
									}
								}
							}
						}
...
$pdf_content='
			...
			<tr bgcolor="#006" style="color:#FFF"><td colspan="3" align="left">Leave Request Copy Of : '.$stuname.'</td></tr>
	 		</table>
			<table>
			<tr><th>Student Name : </th><td>'.$stuname.'</td></tr>
			<tr><th>Student Department : </th><td>'.$dept.'</td></tr>
			...
			;
			$name = $user.$leavedate.'to'.$end.'.pdf';
			$reportPDF = createPDF($pdf_content, $name);
				}
				else
					{
					header('location:leaverequest.php?err='.urlencode('Start Date is invalid !'));
					}
			}
		
		else
			{
			header('location:leaverequest.php?err='.urlencode('Pl. Enter some details !'));
			}
	}
	else
	{
